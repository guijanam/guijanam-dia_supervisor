...
### 🔑 API 키 취급 규칙 (반드시 지킬 것)

RLS 가 `using (true)` 인 현 구조에서는 **키를 아는 사람 = 그 승무소
데이터 전체를 읽고 쓸 수 있는 사람**이다. 키 하나가 곧 그 승무소의
전 직원 개인정보(이름·사번·PIN 해시)다. 따라서:

1. **키를 코드에 하드코딩하지 않는다.** 반드시 환경변수를 거친다
   ([lib/supabase.ts](../lib/supabase.ts) 가 유일한 접점).
   → 2026-08, 레거시 단독 HTML(`coworker_month.html`)에 운영 anon key 가
     하드코딩된 채 **공개 레포에 커밋되어 있던 사고**가 있었다. 해당
     파일은 삭제하고 키는 교체했다. 같은 실수를 반복하지 않는다.
2. **새 승무소는 처음부터 publishable key(`sb_publishable_...`)를 쓴다.**
   기존 anon key(JWT, `eyJ...`) 방식은 2026년 말 지원 종료 예정이다.
3. **secret key / service_role key 는 만들지도, 넣지도 않는다.**
   이 앱은 service_role 을 쓰지 않는 설계다(`migrations.sql` 섹션 5 주석).
   `NEXT_PUBLIC_` 값은 **브라우저 번들에 그대로 박히므로**, 여기에
   secret 계열 키가 들어가면 즉시 전체 권한이 공개된다.
4. **키가 유출되면 새 키 생성만으로는 끝나지 않는다.** Supabase 는 새 키를
   기존 키 *옆에* 추가할 뿐이므로, 옛 키는 대시보드
   **Settings → API Keys → Disable** 로 명시적으로 비활성화해야 죽는다.
   (비활성화는 되돌릴 수 있다 — 놓친 클라이언트가 있으면 재활성화 가능)

   유출 시 순서: **새 키 생성 → Vercel env 교체(3개 환경) → 재배포 →
   동작 확인 → 옛 키 Disable → 옛 키로 호출해 401 확인**
